...
**file-loader**在打包时会做3件事：

1. 随机生成一个hash值，或者用指定的文件名
2. 将图片拷贝到输出目录的静态资源文件夹下 
3. 将打包后的图片名称返回给require函数
...
function parseQuery(query) { // name=zwh&age=18
...
**url-loader**是在**file-loader**基础上封装的，在打包时会做3件事：
