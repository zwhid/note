...
then方法的onFulfilled/onRejected函数作为参数是可选的，如果没有提供处理函数，由最后一个then的函数处理。
...
onFulfilled没有参数时默认是`val => val`，把成功值传递给下一个then

onRejected没有参数时默认是`err => { throw err }`，把失败值抛出，下一个then会拦截到